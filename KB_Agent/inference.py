...
def resolve(clause0, clause1, literal):
    ...
    if any(negate(other) in clause1 for other in clause0):
        return None
...
class KnowledgeBasedAgent:
    ...
    def choose_location(self, x, y, cave_size):
      ...
      safe = self.safe()
      maybe_unsafe = self.maybe_unsafe()
      unvisited = self.unvisited()

      unvisited_safe = unvisited - maybe_unsafe

      if unvisited_safe:
          return random.choice(list(unvisited_safe))
      elif safe.intersection(unvisited):
          return random.choice(list(safe.intersection(unvisited)))
      elif maybe_unsafe:
          return random.choice(list(maybe_unsafe))
      elif unvisited:
